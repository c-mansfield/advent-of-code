...
class CubeConundrum {
    ...
    fun sumPossibleGames(input: List<String>): Int = parseGames(input)
            .filter { c -> c.rounds.all { it.blue <= 14 && it.red <= 12 && it.green <= 13 } }
            .sumOf { it.id }

    fun sumPowerGames(input: List<String>): Int = parseGames(input)
            .map { c -> c.rounds.maxOf { it.blue }  * c.rounds.maxOf { it.green } * c.rounds.maxOf { it.red }}
            .sumOf { it }

    private fun parseGames(input: List<String>): List<Game> = input
            .map { it.split(": ")}
            .map { (game, sets) -> Game(game.split(" ")[1].toInt(), parseRounds(sets)) }

    private fun parseRounds(game: String): List<Round> = game.split("; ")
            .map { it.split(", ") }
